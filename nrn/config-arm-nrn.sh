...
CC=${MYAGCC} CXX=${MYAGCC} CXXFLAGS="-I${NDKDIR}/sources/cxx-stl/gnu-libstdc++/include -I${NDKDIR}/sources/cxx-stl/gnu-libstdc++/libs/${ARCH}/include -fno-rtti -fno-exceptions" LDFLAGS="-L${NDKDIR}/sources/cxx-stl/gnu-libstdc++/libs/${ARCH} -lstdc++" ./nrn-hg/configure --enable-static --disable-shared --without-x --without-nmodl --without-memacs --prefix=${TARGET} --host=x86-linux --build=arm-eabi
