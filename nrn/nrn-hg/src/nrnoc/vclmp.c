	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  hoc_register_dparam_size(_mechtype, 2);
 	...
 	ivoc_help("help ?1 VClamp /home/cs/neurodroid/nrn/src/nrnoc/../.././nrn-hg/src/nrnoc/vclmp.mod\n");
...
double icur (  )  {
 ...
 if(error){fprintf(stderr,"at line 141 in file ../.././nrn-hg/src/nrnoc/vclmp.mod:\n		SOLVE clamp\n"); nrn_complain(_p); abort_run(error);}
 _licur = ( vo - v ) / rstim ;
     }
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  update();
 if(error){fprintf(stderr,"at line 111 in file ../.././nrn-hg/src/nrnoc/vclmp.mod:\n	SOLVE update METHOD after_cvode : but not really (see above)\n"); nrn_complain(_p); abort_run(error);}
