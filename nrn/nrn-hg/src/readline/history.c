...
#if NO_ALLOCA
#define USED_SIZE 128
  char ** saved_alloc;
  int allocs_used = 0;
  int allocs_alloced = USED_SIZE;

  saved_alloc = (char **) malloc(USED_SIZE * sizeof(char *));
#define save_an_alloc(ptr) \
  if (allocs_used >= allocs_alloced) \
    saved_alloc = (char **) \
      realloc(saved_alloc, (sizeof(char *) * (allocs_alloced += USED_SIZE)));\
  saved_alloc[allocs_used++] = ptr
#define free_the_allocs() \
  while (allocs_used > 0) \
    free(saved_alloc[--allocs_used]); \
  free(saved_alloc)
#endif
